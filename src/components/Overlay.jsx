...
const Overlay = ({handleVisibilityClick, currencyOptions, visible, handleChangeClick}) => {
  return(
      <div id={'overlay'} className={`overflow-y-auto backdrop-blur-[4px] h-screen fixed z-40 bg-[#161821d9] w-screen top-0 left-0 text-[1.6rem] ${visible ? 'visible' : 'hidden'}`}>
                  ...
                  <div className={'min-h-[470px] bg-add rounded-[16px] flex flex-col gap-[1.6rem] max-w-[59.2rem] py-[3.9rem] px-[3.2rem] relative w-full'}>
                      ...
                      <div className={'flex flex-col relative '}>
                          <div className={'bg-main rounded-[8px] h-[5.2rem] px-[2.4rem] flex items-center'}>
                              <input placeholder={'Поиск'} className={'bg-transparent focus:outline-none h-full w-full leading-[1]'}/>
